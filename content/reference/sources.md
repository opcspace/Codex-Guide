---
title: 官方资料索引
description: 本知识库使用的 OpenAI 官方 Codex 文档、课程与开源资料。
---

# 官方资料索引

本库只把 OpenAI 官方站点、OpenAI Academy 和 `openai/codex` 官方仓库作为事实来源。内容为中文整理与实践转译，不是官方文档镜像。

**最后核查：2026-07-13（Asia/Shanghai）**

## 产品与入门

- [Get started with Codex](https://openai.com/codex/get-started/)
- [What is ChatGPT Codex?](https://openai.com/academy/what-is-codex/)
- [How to get started with ChatGPT Codex](https://openai.com/academy/codex-how-to-start/)
- [Codex Academy hub](https://openai.com/academy/codex/)
- [Codex CLI](https://learn.chatgpt.com/docs/codex/cli)
- [Codex IDE extension](https://learn.chatgpt.com/docs/codex/ide)
- [Codex cloud](https://learn.chatgpt.com/docs/cloud)
- [ChatGPT desktop app](https://learn.chatgpt.com/docs/app)

## 工程实践

- [Best practices](https://learn.chatgpt.com/guides/best-practices)
- [How OpenAI uses Codex](https://openai.com/business/guides-and-resources/how-openai-uses-codex/)
- [Custom instructions with AGENTS.md](https://learn.chatgpt.com/docs/agent-configuration/agents-md)
- [Code review](https://learn.chatgpt.com/docs/code-review)
- [Codex code review in GitHub](https://learn.chatgpt.com/docs/third-party/github)
- [Worktrees](https://learn.chatgpt.com/docs/environments/git-worktrees)
- [Local environments](https://learn.chatgpt.com/docs/environments/local-environment)

## 扩展与自动化

- [Automations](https://openai.com/academy/codex-automations/)
- [Non-interactive mode](https://learn.chatgpt.com/docs/non-interactive-mode)
- [Codex GitHub Action](https://learn.chatgpt.com/docs/github-action)
- [Build skills](https://learn.chatgpt.com/docs/build-skills)
- [Model Context Protocol](https://learn.chatgpt.com/docs/extend/mcp)
- [Codex 103: Advanced Workflows and Automation](https://academy.openai.com/public/clubs/builders-etkn1/resources/codex-103-advanced-workflows-and-automation-2026-03-18)

## 安全

- [Sandbox](https://learn.chatgpt.com/docs/sandboxing)
- [Agent approvals & security](https://learn.chatgpt.com/docs/agent-approvals-security)
- [Agent internet access](https://learn.chatgpt.com/docs/cloud/internet-access)
- [Running Codex safely at OpenAI](https://openai.com/index/running-codex-safely/)

## 开源实现

- [openai/codex](https://github.com/openai/codex)
- [openai/codex AGENTS.md](https://github.com/openai/codex/blob/main/AGENTS.md)

## 收录原则

1. 官方文档优先于帮助中心旧文章和第三方教程。
2. 涉及命令、权限、产品入口时，以核查日的官方页面为准。
3. 实践建议会与事实分开表述；推导内容不冒充官方原话。
4. 不大段复制官方内容，只做摘要、重组和可执行化。
